...
export class LoginService {
    ...
    refreshToken(): Observable<Loginned> {
        return this._baseService.send<Loginned>(this.ser, "refreshtoken", { refreshToken: this._userService.user.login.refreshToken },
                                                                          { headers: {'UUID': this._uuidService.uuid, 'skip':'sk' } } ).pipe(
            tap(res => {
                this._userService.localSetLoginTokens(<Loginned>res);
            },
            err => {
                this._userService.localLogout();
            })
    ...
    createRefreshTokenRequest(): HttpRequest<APIResult> {
        return this._baseService.createRequest(
            this.ser,
            "refreshtoken",
            {refreshToken: this._userService.user.login.refreshToken},
            {
